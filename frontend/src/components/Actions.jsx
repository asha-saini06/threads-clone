  ...
  const handleReply = async () => {
    ...
    try {
      const res = await fetch("/api/posts/reply/" + post._id, {
        ...
        body: JSON.stringify({ text: reply }), // send the reply as a string
      });
      const data = await res.json();
      console.log(data);
      if (data.error) return showToast("Error", data.error, "error");
      ...
      setReply(""); // clear the reply state after replying
    } catch (error) {
      showToast("Error", error.message, "error");
    } finally {
      setIsReplying(false);
          ...
          <ModalBody pb={6}>
            <FormControl>
              <Input
                ...
                value={reply} // set the value of the input to the reply state
                onChange={(e) => setReply(e.target.value)} // update the reply state when the input changes
              />
            </FormControl>
          </ModalBody>
